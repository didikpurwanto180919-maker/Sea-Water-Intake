...
# ==========================================
# 4. FUNGSI KIRIM WHATSAPP (FONNTE API)
# ==========================================
...
# Tombol Test WhatsApp Manual di Dashboard Utama
st.markdown("---")
col_wa_test1, col_wa_test2 = st.columns([3, 1])
with col_wa_test1:
    st.markdown("### 🧪 Uji Koneksi Gateway WhatsApp")
    st.markdown("Gunakan tombol di samping untuk mengirimkan pesan uji coba langsung ke nomor HP Shift Operator.")
with col_wa_test2:
    if st.button("🚀 Kirim Test WhatsApp", type="primary"):
        with st.spinner("Mengirim pesan WhatsApp..."):
            test_pesan = (
                "🧪 *TEST PESAN JELLY-MARVEL PLTGU GRATI*\n\n"
                "Sistem peringatan dini beroperasi normal.\n"
                f"Status saat ini: {'KRITIS' if risk_class==2 else ('WASPADA' if risk_class==1 else 'AMAN')} "
                f"(Risiko: {display_score:.1f}%)"
            )
            res = kirim_whatsapp(wa_token, wa_target, test_pesan)
            if res.get("status"):
                st.success("✅ Berhasil terkirim!")
            else:
                st.error(f"❌ Gagal: {res.get('reason', 'Periksa token/device')}")
...
if wa_active and risk_class == 2 and not manual_override:
    ...
    if st.session_state.get("last_sent_hour") != current_hour_key:
        pesan_darurat = (
            ...
            "Harap segera jalankan SOP Mitigasi & Mandatori Shift Operator!"
        )
        ...
        if result.get("status"):
            ...
            st.sidebar.error(f"❌ Auto WA Gagal: {result.get('reason', 'Cek token')}")
